 ...
 *
 * | giant folk | 59,641,286 | **+0 B, and the zero is the interesting part.** Eighteen zones' interiors went from a 3 m ceiling to 6 m (`roomScene.GIANT_FOLK_ZONES`), which is a **second course of the same wall module** — `interior.WALL_COURSE`, seated 3 m up. Measured over the whole world: **+27,328 wall placements**, 249,885 village chords to 277,213, across 1,143 dressed rooms of the 10,799. Not one of them is a new *bucket*: a second course is the same `(model, texture)` pair at a different `y`, so it lands in a wrapper the pool already pre-warmed and the ledger — which counts geometry, instance buffer and rigs, all decided in the constructor — cannot see it at all. What did move is instances **inside** those wrappers: the world's worst wall bucket goes 12 to **22** of `WRAPPER_CAPACITY`'s 32 (eleven plain chords over two courses, in IceCrag Castle), and `pool.INTERIOR_WRAPPER_CEILING`'s derivation is unchanged at eleven because its terms are models and primitives rather than chords. Ten instances of slack left, which is what would have to be spent before this row ever became a number. Zero geometry (no new model), zero materials, zero programs, zero rigs. |
 *
  ...
  it(`walks ${TRAVERSAL_ROOMS} rooms of the built world without allocating`, () => {
    ...
    outer: for (const zone of zones) {
      ...
      for (const room of order) {
        ...
        // The room's floor, dropped and picked up. Half of them carry a rot clock past their warning
        // threshold, so the fade ladder is actually climbed rather than merely reachable — a walk that
        // only ever drew step 0 would leave seven of the eight materials unexercised.
        const floor = visited % FLOOR_CYCLE;
        for (let i = 0; i < floor; i++) {
          const rotting = (visited + i) % 2 === 0;
          entities.upsert({
            id: nextGroundId--,
            kind: 'item',
            name: 'a dropped thing',
            sprite: 'item_weapon',
            x: (origin.tx + ROOM_TILES / 2) * TILE_SIZE,
            y: (origin.ty + ROOM_TILES / 2) * TILE_SIZE,
            facing: 'south',
            ...(rotting ? { remainingMs: 12_000 * (i + 1), warnAtMs: 60_000 } : {}),
          });
        }
