...
describe('the pool key set', () => {
  it('has exactly seven built shapes, and the trees and the kit on top of them', () => {
    ...
    // **And nothing for the animated objects, since `glint.ts` retired the one there was.** The list
    // and its derived keys are kept empty rather than deleted — see `ANIMATED_MODELS` — so this pins a
    // zero, which is the assertion that would catch a rigged model being added without the stand-in
    // and dress-call plumbing its own branch needs.
    assert.equal(ANIMATED_MODELS.length, 0);
    assert.equal(ANIMATED_GEOMETRY_KEYS.length, 0);
  ...
  it('has exactly 328 materials, and the arithmetic is legible', () => {
    ...
    // Characters: 14 atlases, keyed by texture alone rather than by `(model, texture)` — a body
    //   material carries no per-model uniform, so 28 models share fourteen materials. None fade.
    ...
    // Animated objects: 0. This row held **8** from `a7708d2` until the glint — the loot sparkle's
    //   rot-fade ladder, one shared material a rung, because a `SkinnedMesh` has no per-instance
    //   channel and could only be handed a different material. `glint.ts` fades in a shader against a
    //   per-emitter deadline, so the ladder is gone and so is the model: see `ANIMATED_MODELS`.
    ...
    assert.equal(
      ...
      terrain * 2 +
        ...
        // And the animated objects, which have no material key of their own at all while the list is
        // empty. The term is kept so the day one returns, this sum is the line that has to say how.
        ANIMATED_MODELS.length * 0,
    );
  });
  ...
  it('costs nine programs for all 328 materials, plus two for the foliage shadows', () => {
    ...
    // **And the floor's glint is not in this count at all, which is the honest reading.**
    //
    // From `a7708d2` until the glint, the loot sparkle's eight fade rungs lived on the *character*
    // recipe precisely so they would land on this skinned program rather than compile a tenth —
    // `USE_SKINNING` is an object define and no furniture is skinned. `glint.ts` has no pooled material
    // at all: it owns a `ShaderMaterial` of its own, built in its constructor and added to the scene
    // beside the rain's and the snow's, which `programKeys` cannot see and never could. So the pool's
    // number stays nine and the browser's is one higher — the same arithmetic weather has always had.
