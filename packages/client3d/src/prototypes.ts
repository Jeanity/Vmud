 ...
 *
 * **Empty today, and kept rather than deleted.** It is separate from {@link OBJECT_MODELS} for the
 * reason that list is separate from {@link PROPS_MODELS}, one step further along — the difference is
 * not *what a room may contain* but **how it is drawn**:
 *
 ...
 * texture, so two instances of one skinned mesh are two copies of the same pose — the exact argument
 * `pool.BODY_POOL_SIZE` makes about bodies. That is expensive per entity, which is why the one member
 * this list ever had is no longer in it.
 *
 * **It held `loot_sparkle`**, a rigged 420-triangle glint that stood over every thing lying on a
 * floor: seven joints, one `Skeleton`, one `AnimationMixer` and a cap of 41 per client. `glint.ts`
 * replaced it with a particle field — one `InstancedBufferGeometry`, one draw call for every glint in
 * the room, no cap and no per-entity allocation at all. The model, its import
 * (`modelgen.buildAnimatedObject`), its manifest row and its files under `assets/props/loot_sparkle/`
 * and `public/models/props/loot_sparkle/` are all **kept and unused**: the import is verified by
 * `props.test.ts` against the glTF on disk, and the category is the thing the next rigged prop will
 * arrive through. Adding it back is one string in this array.
 */
export const ANIMATED_MODELS: readonly string[] = [];
 ...
 * `props.ts` does with it: `PropsSet.standIn` registers a *box* for every row, and a rigged model
 * wants a real armature under the real joint names or nothing downstream can bind it. Its own list,
 * its own registration; see `props.ts`'s animated branch.
 *
 * Empty while {@link ANIMATED_MODELS} is.
 */
