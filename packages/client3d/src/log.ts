/**
 * **Salvaged verbatim from `packages/client/src/log.ts` (241 lines) at M3.**
 *
 * The plan's §2 predicted this would become *more* valuable in 3D — "combat rolls and room prose are
 * unreadable as floating 3D text" — and it grew from 55 lines to 241 in the meantime, which only
 * makes the prediction more true. Everything below this block is byte-for-byte the donor.
 *
 * Copied rather than shared because it is DOM, and `@mygame/shared` forbids browser APIs
 * (`CLAUDE.md`). Two clients, two identical copies, until M7 retires the Phaser one.
 *
 * **Two comments below name Phaser and are left standing.** `onFocusChange` says "Phaser listens on
 * the document, so it does not care that the keystroke was aimed at an input", and `onLayoutChange`
 * says "Phaser's `Scale.RESIZE` only watches the window". Both are true of the donor and neither is
 * true here — this client's keyboard gate is `Input.typing` and its resize is
 * `WebGLRenderer.setSize` — but they are kept unedited so that a diff against
 * `packages/client/src/log.ts` stays empty and a fix in one file can be applied to the other without
 * reading it. The 3D equivalents are wired in `main.ts`, on the same two callbacks.
 *
 * ---
 *
...
export class LogPanel {
  ...
  write(channel: PanelChannel, text: string): void {
    ...
    if (this.lastLine?.isConnected && this.lastKey === key) {
      this.lastCount++;
      paint(this.lastLine, `${text} (x${this.lastCount})`);
    } else {
