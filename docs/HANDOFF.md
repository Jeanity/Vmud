...
**Items exist, but there is no item *content*.** Since 15b a character has a bag, things can be picked
up, dropped, worn and taken off, and a corpse holds what its owner was carrying. What is missing is
anything to hold: **every item in the world comes from the rolled starter kit** in
`shared/src/equipment.ts`, which is code rather than data. Nothing has been harvested from Duris'
`.obj` files, so **mob corpses hold nothing** — the only way something reaches the floor is a player
putting it there. That is 15c's, with stacking, charges, containers and gold.
