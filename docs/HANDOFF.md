...
**Stacking, charges, containers and gold.** The catalogue carries `stackLimit`, `uses` and a container
rule on every entry since 15c, and **nothing reads them yet** — a stack of arrows is twenty separate
items in twenty slots, a wand has infinite charges, and a quiver is an ordinary thing you can put on
your back. `DESIGN-inventory.md` §3 and §4 are the spec; the fields are there so the work is wiring
rather than re-harvesting.

**Objects do not lie in rooms from the zone files.** `O` (put an object in a room) and `P` (put one
inside a container) are harvested — 773 and 329 commands — and have no executors. Both carry a
world-wide instance limit, and honouring it means counting how many of a vnum exist on floors, in
bags and inside containers. Without that census every repop would add another and the world would
silt up, so they wait on it rather than shipping a leak.
...
**A `wield` verb does not exist**; `wear` handles every slot including weapons. Diku splits them and
the split earns its keep once a character has enough gear for it to save typing — with two-handed
weapons, in 15c.
