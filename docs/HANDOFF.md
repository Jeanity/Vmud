...
| Casting | A wind-up as a `cast` event on the scheduler, revalidated once a second — a changed room, lost footing or a lost target breaks it, free (the price is paid at completion). Registry of four nukes with transcribed dice; **one save per cast** (the ×5-mod trap kept), **one shrug per blow** (players are raceless and never shrug). Mobs cast what `MobTemplate.spells` authors, 50% per round boundary, level-rolled quick chant | Refreshed on every flight so a pursuit cannot be out-waited, paid by mob and player alike, and not charged for a flight from nothing |
...
phases.** 1,614 tests (897 server / 583 shared / 134 worldgen), typecheck clean. The day's landings,
newest first:

- **Phase 20 slices 1–3 — spells, up to and including mob casters** (three table rows): the tick
  drains the scheduler and routes by kind (slice 1, the design note's mandatory first commit); the
  wind-up you can watch and break (slice 2 — the beat *is* the interruption system); and the
  ...
  casting magic missile and burning hands**). The slice-3 drive found one bash silencing a caster
  permanently — `mundane_autostand` transcribed; see its own row. **Left in the phase: slice 4
  scrolls (`recite`, the classless player-casting path), slice 5 heals/buffs via
...
**Next, by the cadence**: round 14's **M is Phase 20 — spells, slices 1–3 landed 2026-08-07**; next
is **slice 4, scrolls** — `recite`, the classless path that finally lets a *player* cast from the
registry (and with it the shrug gate gets its first live workout against MR-race targets). Then
slice 5 heals/buffs, slice 6 areas. Track V has no numbered item left — the bubble cap loose end and
