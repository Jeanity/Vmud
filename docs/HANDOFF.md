...
| Casting | A wind-up as a `cast` event on the scheduler, revalidated once a second — a changed room, lost footing or a lost target breaks it, free (the price is paid at completion). Registry of four nukes with transcribed dice; **one save per cast** (the ×5-mod trap kept), **one shrug per blow** (players are raceless and never shrug). Mobs cast what `MobTemplate.spells` authors, 50% per round boundary, level-rolled quick chant. Players cast by **`recite`** — 135 harvested scrolls, cast at the scroll's level, no gate of any kind, burnt on recital, refused mid-fight | Refreshed on every flight so a pursuit cannot be out-waited, paid by mob and player alike, and not charged for a flight from nothing |
...
phases.** 1,617 tests (897 server / 584 shared / 136 worldgen), typecheck clean. The day's landings,
newest first:

- **Phase 20 slices 1–4 — spells, from the dispatcher to players casting** (four table rows): the
  ...
  casting magic missile and burning hands**); and `recite` (slice 4 — the classless path, 135
  scrolls harvested with their stored levels and spells, the scroll burnt whether or not anything
  answers). The slice-3 drive found one bash silencing a caster permanently —
  `mundane_autostand` transcribed; see its own row. **Left in the phase: slice 5 heals/buffs via
  `joinBySupporting`, slice 6 areas (players-only thinning).**
...
**Next, by the cadence**: round 14's **M is Phase 20 — spells, slices 1–4 landed 2026-08-07**; next
is **slice 5, heals and buffs** — `joinBySupporting` and `THREAT_PER_HEAL` get their first
producers, the affect registry grows `ac`/`hit`/`saves` locations with readers, and the group
roster's exact-pools protocol change lands with the first aimable heal. Then slice 6 areas. (The
