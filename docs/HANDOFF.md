...
| The bow rides the back, the quiver beside it | The launcher half of Phase 15’s drawn-gear tail (2026-08-09). A wielded bow was visible only in the draw — the kit pack ships bows as Shoot sheets alone — so between shots the archer carried nothing. ULPC has the missing half: `weapon/ranged/bow/normal/walk`, wulax’s bow with pvigier’s walk animations, on a **128 px grid** because a slung bow overhangs a 64 px cell (measured: 27 px at the worst frame). Staged as `weapon-bow-carry` (behind the body, the artist’s z −1) and `weapon-bow-carry-front` (a south-only strip for the limb crossing the chest), trimmed to the 9 real columns, loaded at their own frame size via `LPC_STATIC_GEOMETRY` — origin 0.5 centres an oversize frame for free. **The swap is the shoot-only trick inverted**: real walk under a hand-staged blank `-shoot` twin, so the slung bow vanishes the instant the draw pose puts the kit bow in the hands — without the blank, hold-the-walk-frame degradation would show two bows. **First layers ever behind the body**: `characterLayers` painted the body first unconditionally, so ULPC’s behind-body z values were unrepresentable; the body now competes in the sort at `BODY_Z` 10, ULPC’s own plane, and the quiver the artists drew at 8 hangs behind the shoulder as drawn. The worn quiver itself cost no art at all — artgen had already staged `quiver-quiver` for the catalogue — just an `artClassOf` branch (`DURIS_ITEM.quiver` → `quiver`) and a `KIT_ART` row, the shield’s argument a third time. Alignment, row order and filler columns were measured against our staged body before adoption (composite at four facings); melee hold-frames remain the tail’s open half |
| Velen Square lives — Phase 23 | The noticeboard is `boards.c` transcribed onto our seams: a board is a **field on a room** (`Room.board`, a slug) rather than an object in its contents, `read board` lists newest-first with true storage numbers, `read <n>` prints one, out of range earns the source’s own *“That message exists only in your imagination.”* Gods post from the Messaging panel (`/admin/api/boards`), posts persist in `overrides/boards.json` (gitignored beside `settings.json`), caps are the source’s: 199 messages, 70-char headline, 4000 body. Readers are announced to the room — *“X studies the noticeboard.”* The **inn flag** is `ROOM_HEAL` transcribed from the code and not the lying define comment: `limits.c:304` adds level × 2 to hp regen, hp only, status ≥ sleeping — the Anchor & Anvil carries it. Four **authored anchor shops** (Sedric weapons+arrows, Marda armour, Ilvara potions, Osric food in the inn) ride A9b mobs + `shops-authored.json`, read by the same tolerant loader as the harvest, second word wins. Two **watchmen of Velen** stand the north and east posts — passive until the city decides whether the watch enforces law (city doc §6.2). One trap found: the A9 loader refuses negative armourClass and drops the record **silently at boot** — the watchmen were missing until their AC came up to 0 |
| The guilds teach — Phase 24 | `practice.ts` is `do_practice` on our seams: any registered guildmaster in the room teaches, bare `practice` prints the hall’s slate (your standing, the price, or the source’s "(cannot practice)"), a named skill walks the refusal ladder in source order — purse, twice-your-level (both sentences), class ceiling, twice-the-teacher’s-level with the four sassy refusals verbatim — and a surviving lesson pays the curve (×100 into copper, ten silver floor, ~sixty-five silver at mastery) for **+1 learned**. Teachers are `trainers-authored.json` (vnum → class); the hall teaches what `CLASS_SKILLS` grants that class, so cross-class students may buy shared skills and the ranger trains `ranged` behind Vex’s quiet door. The Guild Ward (100003, seamless, portal off the north Spine): Hall of the Shield / the Quiet Door / Seminary of the Dawn / the Wizards’ Stair going **up** — the first vertical authored rooms. Four level-50 masters, deliberately killable (protocol 30’s changelog states it): the `%` badge and *Train* menu row mark them. `prac` reaches it as a prefix; `pr` is the shortest spelling |
| The wall takes its watch — Phase 25 | The measurement was the design: `patrol_leader` is a *bounded wander biased toward trouble* — Duris has no waypoint machinery, `ACT_PATROL` only excludes the mundane wander. The waypoint beat is therefore ours (city ledger’s ask): `patrols-authored.json` holds a route’s **turning points** per vnum, `beginWalkTo` walks each leg through the machinery the hunts own (announcements and arrival settle included), rests count in **pulses** so the beat lives inside the simulation’s clock, and a mob with a beat never mundane-wanders — the source’s one real rule, kept. The Wall Walk (100009): the North Gatehouse off the Hall of the Shield, five battlement rooms at z+1 where a wanderer could never reach (vertical exits are Place changes the wander refuses), a watchman of the wall beating western reach to eastern and back — verified in three API snapshots — and Meraud the fishwife and a dock porter wandering the square below. The seen-when is measurably true |
| The dark half, and the first lock that holds — Phase 26 | The content phase carried a mechanism: **authored zones keep their locks**. `LOCKS_HOLD` stays false for the harvest (42 locked doors, 0 key ids, IceCrag loses 194 rooms if honoured) but `relaxLocks` now skips ids at `AUTHORED_ZONE_BASE`+ — an authored lock comes with an authored key by construction. `unlock`/`lock` transcribe `do_unlock`: the key is a **carried object whose vnum is the door’s `keyId`**, the door must be shut to turn a key in it, both ends move together (a one-way exit’s `far` is optional, as `setDoorClosed` treats it), and the room watches you do it. Vertical doors work for free — transitions consult `world.doorway` and carve no tiles, which is what lets a **grate** be a door. The Shambles (100007) west of the crossing: cutpurse, bravo, ratcatcher, all aggressive, plus a dead end that is trodden very flat. The Underquay (100010) at z −1 under the tanners’ grate: two tunnel dwellers, and the vault behind banded oak — verified locked in the running world. The key (9000002) is loot on the dweller at the tunnel crossing, so the lock is a question with an answer somewhere |
| The roads are stitched — Phase 27, and Act VII closes | **The number to know is 4.** Every worldgen run since Phase 22 reported *0 cross-source edges stitched* — the rule was built, tested and never fired. It fires now: the North Road ends in harvested **431 “A Road Through the Flatlands”** (a road meeting a road; IceCrag’s 15–60 makes the design’s “escorted stretches only” literal), the East Road at **77504** on the Stag Forest’s edge, the South Road at **77580** where the Stump Bog starts, and the Underquay’s smugglers’ tunnel at **81128, the kobolds’ sacrificial pit** — which is why there are bones in it. Each return half was injected into a **free** side; the validator refused a mismatched pair (my fish market entered east while the square left south) and was right. **Measured: 645 rooms across 14 zones walkable from the Great Crossing**, the spawn field 23 steps away — Velen is joined to the world a character already plays in. Harbourside (100006) is the docks: fish market, long quay, warehouse row, the Wet Boot (second `inn` room, Hessa keeps a shop), and a quayside grate into the sewer main. **Rare loads were already data** — `percent` on an authored loot row is validated 1–99 and rolled at spawn (`applyOutfit`’s `dropRoll`), so the roadwarden’s cloak rides a brigand at 8% with no new mechanism. Three quests seed the wave: the watch, the landlord, the armourer |
| Tordraken Castle — the rewrite | **Measurement made this tractable and the roadmap row had feared worse.** Not 217 rooms of ice prose: **72 rooms carrying 52 distinct (name, prose) pairs**, because duplicated rooms share text and two thirds of the castle was already weather-neutral. Better, the castle was **already a ducal keep** — guard walks on four walls at two floors, marble galleries, banquet hall, archives, barracks, an observatory, a Viscount, an Archivist, treasury guards, masons and a cleaning crew — so **40 of 45 placed mob kinds needed nothing at all** and only six were re-themed. Two accidents in our favour: the harvested prose already looked west to the Sword Coast and south to Calimshan, which is exactly where Velen stands. The translations: Auril’s snowflake becomes Draknor’s dragon-head (Tordraken = dragon tor, and Velen sits on the Dragon’s Head), the frozen cascade becomes a real waterfall, the Ice Garden becomes the **Salt Garden** (same ghastly paradox, brine instead of frost), the Court of the Icess becomes the **Ducal Court**, and the woman in the ice block becomes the founder kept in a glass-and-lead case of brine. Strife holds the keep and styles herself Duchess; Malice keeps his name and his mother, because he is slice 7’s boss and carries Whisperwind. **Carried mechanism: `zone-overrides.ts`** — a harvested zone’s *name* was the one field no overlay could reach (rooms.json stops at rooms, zones-authored.json names only authored zones), composed at boot beside the room overrides. **The test is a sweep**: zero rooms still carry an ice word, checked by rule rather than by eye. One trap: an em dash written through a Python script came back as U+FFFD in twelve rooms — authored prose in this project stays ASCII, and the sweep now proves that too |
| Velen’s first stones — the authored world (Phase 22) | Act VII opens. `data/authored/zones/<id>.json` is first-party zone source, **committed to git** where the harvest never is, in the emitted `Zone` shape — no second dialect to translate. Worldgen merges it beside the harvest on full builds (announced skip on `--zone` builds) under a validator that checks everything a hand can get wrong: the id law (zones at `AUTHORED_ZONE_BASE`+, **rooms at zone × 1000 + n** — nine digits, names its zone on sight), the SECTORS/ROOM_FLAGS catalogues against typos, cell collisions, geometric consistency or an honest portal, and **no door you cannot come back through**. The **cross-source edge rule** is checked at merge time when both worlds are in hand: portal or refused, the return half **injected** into the harvested room in the emitted output, and never over an existing exit — `links.json`’s three rules, graduated from overlay to pipeline (links.json stays, for *runtime* authoring). `GameWorld.load` routes authored ids **file-first** — a built committed zone loads exactly as a harvested one; only file-less ids fall back to A8d’s shell — and the A8d zone allocator vaults past committed ids so the panel’s next zone cannot silently become a district. The seed content: **Velen** (the owner renamed the city from Ironquay the same day, choosing the real Toril port — see DESIGN-city.md §1), zone 100001 “Velen - The Square”, six rooms at the great crossing laid to TorilMUD Waterdeep’s street plan by the owner’s ask, first-party prose that ships, the Anchor & Anvil behind the first authored door. An island until Phase 27’s roads — reach it by admin teleport. 15 worldgen tests hold the refusals |
