...
- **1,264 tests** (701 server, 468 shared, 95 worldgen), typecheck clean across all five packages.
...
### Start here — round 7, A8 slices 1 and 2 are in (2026-08-05)

**A8 slices 1 (infill) and 2 (deletion) landed 2026-08-05** — see the two table rows. What is left of
A8 is its **third and last** slice: **extent changes**, with decision 2's explicit `seen`
invalidation. `DESIGN-zone-geometry.md` puts it last because it is the only one that can take
something away from a player who was not consulted, and both slices so far have been able to sidestep
it entirely — `placementRefusal` will not widen a grid and `removalRefusal` will not narrow one, so
nothing built yet can shift a saved tile index. **That is the property slice 3 has to give up**, which
is why it needs the invalidation to be explicit, announced, and per-Place.

**The drive left nothing behind.** The room built in slice 1's drive was removed again by slice 2's,
and the harvested room removed to prove the orphan report (41300, The Pantry) was restored by clearing
its tombstone. `rooms-authored.json` is therefore empty **except for `next: 1000001`**, which is not
noise: it records that id 1000000 has been handed out, and the counter must never hand it out again
even though the room is gone. Deleting the file would reset that.
...
**A8's first two slices**; what is left there is A4c, A7e, A7f, A7d-bag and A8's last slice.
