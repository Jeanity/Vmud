...
| VII — Velen | 22–27 | **0 of 6** — designed 2026-08-08, [DESIGN-city.md](DESIGN-city.md) |
...
#### Phase 20b — Accounts and login ✅ **done 2026-08-08**

Promoted out of §4's parking lot on its own row's argument — *the most misplaced thing on the
schedule* — and built whole in a day. [DESIGN-accounts.md](DESIGN-accounts.md) settles the four
open questions: scrypt via Node's own crypto (no new dependency, parameters ride each hash); **no
email**, so the admin API is the reset path (`GET /accounts`, `POST /accounts/<slug>/password`,
`POST /accounts/<slug>/claim`); account files beside the players', git-ignored, never committed;
and accounts-before-bind honoured by leaving the bind exactly where it was.

**Protocol 23** replaces `hello`: `auth` (credentials or a resume token) → `account` (your
characters, the sixteen cap from `account.h:15`, a fresh resume token) → `enter` (pick or mint a
body) → the old `welcome` burst unchanged. `authFailed` refuses without hanging up; five failures
hang up. Pre-account saves are unowned flotsam — claimable only over loopback, refused to everyone
else, and the admin claim endpoint is what assignment becomes the day the bind opens. The client
grew its first DOM screen — credentials, then the character picker — and a tab's resume token
keeps F5 feeling like the game did before there were passwords.

**The character name law** landed mid-build by owner's rule (see §4's intake row): letters only,
2–12 of them, `_parse_name`'s own reserved list, rude roots matched through their evasion
spellings, and the famous names of the Realms refused outright. Minting only — the flotsam keeps
its digits.

**Seen when:** a wrong password is refused on a socket that survives to try again, and the picker
...
- **Slice 2 ✅ 2026-08-08** — circles and slots: a caster knows their class list as circles open
  (five-level cadence), each cast **debits a casting at completion** (the source's pay-then-fizzle
  order), `rest` commits them back one per twenty unbroken seconds, `spells` reads the book with
  castings left, and INT/WIS finally size the mana pool by class, modifier and blood. The roll
...
- **Slices 4-7 ✅ 2026-08-08, the tail in one evening.** **Skills by temperament**: the
  ...
  the sheet), floors clamped under low ceilings, driven with the sorcerer's refusal. *(**Superseded
  the same evening**: the four groups were re-keyed to the nine classes on the owner's ask. The
  vocabulary survived intact — zero still means the training never happened — but the grants are
  now per class, because a group row hands a skill to every class in it. See the parking lot.)*
...
to zones), identical validation plus one new rule: a cross-source exit (an Velen road ending
...
**Velen** joining the reserved-names law.
...
- **A10 — Start, stop and restart the server** ✅ **done 2026-08-08.** The parking lot's supervisor
  row, built as the row described it: a **separate process** that owns the game server as a child,
  serves `/supervisor/api` on its own loopback port (`SUPERVISOR_PORT`, 8790), restarts it when it
  crashes, and keeps the last 1,000 lines it said. `server/src/supervisor.ts` is the process,
  `server/src/supervisor-policy.ts` the decisions, `admin/src/sections/supervisor.ts` the Server tab.

  **Seen when:** the panel tells you the server is down while the server is down ✅ — with the game
  server stopped, `/admin/api/status` answered nothing at all and the Server tab, through the same
  panel origin, answered **200 · stopped by the operator**. That is the whole feature in one
  comparison, and it is why lifecycle could not be a route on the thing it restarts.

  Five things the build settled, three of them the row's own open questions.

  1. **It owns the game server and nothing else.** The row worried that `npm run dev`'s three
     children under `concurrently` would leave "a restart pointing Vite at nothing". The answer is
     that the two Vite servers are *not* the supervisor's business — they hold no state and survive a
     restart untouched, and the panel staying up **while the game server is down** is the entire
     point. `npm run dev:supervised` is the operator's arrangement (supervisor + the two Vite
     servers); `npm run dev` is unchanged for editing code. Nothing runs two things that start a game
     server.
  2. **It spawns without `node --watch`.** A watcher under a supervisor is two things claiming one
     job: `--watch` restarts the process on a file change without the parent seeing an exit, so the
     pid on screen would be a lie and the crash counter would never move.
  3. **A signal is not enough, and Windows is why.** The row asked for graceful shutdown because
     `PlayerStore` flushes on a debounce. The game server has flushed on `SIGINT`/`SIGTERM` since
     Phase 1 — but **that handler cannot be reached from a parent on Windows**, where
     `child.kill('SIGTERM')` is `TerminateProcess` and no handler runs. So the polite request goes
     over the **IPC channel**, with the signals as the escalation behind it: ask, `SIGTERM`,
     `SIGKILL`. Measured: a stop exits **code 0** in 395 ms with no escalation, where a forced kill
     exits `4294967295`. The exit code is the proof the flush ran.
  4. **The crash-loop rules reset on a healthy run**, and that is the one that would have gone wrong
     silently. Without it the failure counter is a lifetime tally, and a server that ran for a week
     and crashed once would be judged against restarts from the week before. Sixty seconds up starts
     the count again. Backoff is 1-2-4-8-16 s and it **gives up after 5**, because a supervisor that
     retries for ever writes the same failure into the ring a thousand times and pushes the first —
     the only one with the real cause above it — out the back. Driven end to end: five kills by pid,
     the ladder walked in order, then *gave up after 5 consecutive crashes*; Start clears it.
  5. **The token stays optional in dev, and the argument had to be made rather than inherited** —
     the row noted this is now the most security-sensitive surface in the project and wanted the
     token *mandatory*. It rests on one property, enforced by construction: **the argv is fixed in
     the file**. No route accepts a command, so the supervisor starts exactly one program with
     exactly one set of arguments, and a token nobody set costs an unauthorised loopback caller a
     restart rather than a shell. Loopback bind, loopback check and the mandatory `x-admin-token`
     *header* (the CSRF defence, §3) all hold regardless. The day it binds off loopback that argument
     expires with the bind, and `requireToken` is the one place that changes.

  **The third of the row's conditions is honoured too**: a stop or restart announces itself
  world-wide through A2's own endpoint first, best effort and skippable with `{"announce": false}` —
  ...
  `/health`, because *"restart?"* with no number is a question an operator answers wrongly.

  **`supervisor-policy.ts` imports nothing, and that is a requirement rather than tidiness.** The
  supervisor exists to be running when the game server is not — including when it is not running
  *because it does not parse*. Reaching for `admin.ts`'s `LOOPBACK`, or anything through
...
later. **The 3D client is no longer on this list** - see the intake row below and
[PLAN-3d-migration.md](PLAN-3d-migration.md). The rule it rests on, no pixels in `shared` or
`server`, is already CLAUDE.md law and is what makes the turn cheap.
...
| Idea | Verdict | Where |
| --- | --- | --- |
| **Ranged weapons — bows for rangers, thrown knives for rogues, firing into the next room to pull one mob out of a crowd** (owner, 2026-08-08, eleventh ask) | **Agreed, planned, and not built — the owner asked for the plan so Fable can take it tomorrow.** Full design in [DESIGN-ranged.md](DESIGN-ranged.md), six slices, written before any code because **four of its five findings change what “transcribed” means**. The two that reshape the feature: **(1) there is no bow in Duris' weapon-class ladder at all** — ranged is an item *type* (`ITEM_FIREWEAPON 6`, `ITEM_MISSILE 7`, `ITEM_QUIVER 30`), all three of which `DURIS_ITEM` already names, so `weaponSkillFor` is the wrong seam and must not be given a fake class; and **(2) only 17% of the world can be pulled** — measured over all 1,503 templates in the populated zones, **1,248 (83%) carry `pursuit.tier: sentinel` with `trackRooms: 0`** and will never follow anyone. Duris does not hit this because its ranged retaliation uses `HUNT_JUSTICE_INVADER`, which bypasses the hunter gate entirely. Also found: `do_throw` is a stub whose skill `SKILL_RANGE_WEAPONS` is **never created and never granted to any class**, so throwing must be modelled on `do_fire`; every one of the 108 ranged items in our world is already parsed and then discarded by four type gates in `objects.ts`, so the data is a harvest edit rather than authored content; and the bow art exists upstream but is refused by `isWalkSheet`'s 576×256 measurement (its walk is a 1664×512 `walk_128`), while its 832×256 `shoot` sheets already pass `actionGeometry`. One skill covers the whole ask — `SKILL_CREATE("ranged weapons", SKILL_ARCHERY)`, ranger 100 and rogue 75 from level 1, nobody else | **Unblocked — the pivot was decided the same day: `provoked`.** A mob shot from outside its room gets a temporary affect lifting `trackRooms` to exactly 1, expiring on its own harvested `giveUpMs`, then walks home; `staysInZone` and `respectsSafeRooms` are untouched. Recorded as an **affect rather than a fourth `pursuit.tier`**, which is a refinement of the recommendation with identical behaviour: a tier is a harvested trait and nothing in the `.wld` data produces “provoked”, so a fourth tier would be a field only runtime can set — and leaving the 1,248 sentinels sentinel on their own sheet keeps it readable that a shopkeeper does not chase people. **Being shot provokes you; it does not change what kind of creature you are.** The one-room cap is the safety property: a player cannot tow a guard across a zone by shooting it repeatedly. The other requirement — ranged aggro must never reach room-mates — stands as a restriction, and both it and the cap get a test written before the feature, because neither failure shows on a happy path. Slice 1 (un-gate the harvest) depends on nothing and can start immediately |
| **Give the paladin a chest piece worth wearing — and offset it across the other classes** (owner, 2026-08-08, tenth ask) | ✅ **Built as a nine-class table, because the ask carried its own constraint: *"we don't need 1 buffed class that everyone is going to want to play. we will have to give the other classes something equivalent to offset the inbalance."*** Measured first: the paladin was **already 29% ahead** of every other class on gear before any chest piece was added — the request was to widen a gap that existed. Underneath it sat a worse problem the nine-class re-key had created: the common `mainHand` rolls at random and most classes now have most weapon skills at 0, so a **cleric, shaman, necromancer or rogue had a 75% chance of starting with a weapon they could never train** (ranger 25%, sorcerer 50%, druid 25%). That correctness half mattered more than the balance half. `CLASS_KIT` now carries all nine; every `mainHand` maps to a skill the class has. Balance is argued in the only currency level 1 has — weapon skill is +0 for everyone at level 1 and dual wield swings 0% of rounds, so a kit is worth armour and weapon damage and nothing else. Fitness `F = 20·D/(11−AC)`: martials 18.52–19.00 (2.6% spread), casters 17.52–17.85 (1.9%), common table 19.11. The ~6% martial lead is the one deliberate inequality, paid for by circle-1 spells the four martials do not have. The paladin's sword dropped to 2d4 to buy the mail | Done. Two traps caught on the way: `KIT_ART` maps ids to sheets and **an id with no row does not draw**, so `mail_shirt` ships with its mapping to the staged `torso-chainmail` or the chest piece would have been invisible; and the `STARTER_KIT` docblock **claimed AC 19 is hit ~30% of the time when it is 10%**, an error that under-prices armour by 2× and that a design pass built on before it was caught — corrected in place. Guarded by two tests: no class may roll an untrainable weapon, and no class may lead on both armour and damage |
| **The paladin should start with a longsword and shield** (owner, 2026-08-08, ninth ask) | ✅ **Built, and it is the eighth ask's other half rather than a new idea.** Granting a paladin one-handed weapon skills and then rolling them a random dagger and no shield would have been the training without the kit. `STARTER_KIT` was slot-keyed and class-blind, so this adds `CLASS_KIT`: a sparse per-class table that **overrides** the common choices slot by slot, with the paladin its only entry — a plain steel longsword and a battered kite shield. The blade is `weaponClass` 5, which is the load-bearing part: `weaponSkillFor` reads it and returns `slashing-1h`, so the starting weapon trains the skill the ruling granted from the first swing. Neither piece is an upgrade in disguise — the sword is `2d6`, matching the club, the heaviest of the four common weapons, and the shield is **`0..2`** — trimmed from `1..3` on the owner's call the same evening, making it the table's lightest band, so a paladin's armour floor equals everyone else's and only their ceiling moves. Only those two slots are fixed; everything else still varies, so a paladin is not the one class with a cookie-cutter opening hand. The class reaches the roll as an argument to `Simulation.spawn` rather than off `player.identity`, which `restoreProgress` hydrates *after* the kit is rolled — reading it there would have seen `undefined` for every character ever made | Done. Driven live: a fresh paladin spawns holding both, the shield **removes and re-wears** (the off-hand dual-wield refusal is keyed on the explicit `wield X offhand` form, so `wear shield` is untouched by it — verified rather than assumed, since a one-way door would have made the kit a trap), a fresh warrior gets no shield, and at level 27 the starter sword swings at **+17 = 13 level + 4 skill**, identical to a catalogue longsword |
| **The paladin needs one-handed weapon skills — they are shield users** (owner, 2026-08-08, eighth ask) | ✅ **Built, and it is a deviation rather than a correction, because the source meant it.** The re-key surfaced that `skills.c` gives `CLASS_PALADIN` no 1h row at all — both 2h rows, none of the four 1h — and `paladins.h:4` settles that this is deliberate: `IS_PALADIN_SWORD(obj)` is `IS_SET(obj->extra_flags, ITEM_TWOHANDS) \|\| obj->value[0] == WEAPON_2HANDSWORD`, so a paladin's sword in Duris **is** a two-hander by definition, and `bash()` pays a shieldless paladin a bonus for wielding one. Duris' paladin is a two-handed class. The ruling, verbatim: *"they need 1 handed as they are big shield users according to the official DnD description of them. so 1 handed and 2 handed but not dual wield"* — and the project's backbone is the SRD, with the MUD supplying mechanism rather than archetype, so the archetype wins this one. Shipped: all four 1h rows at **95**, mirroring the class's own 2h ceilings because that is the only weapon-skill number `skills.c` gives it and a hand-invented spread across four rows would have nothing under it; 2h untouched; **dual wield still refused**, which the ruling named and the source independently agrees with (no paladin row in `skills.c:3818-3833`). Pinned by a test, because the live danger is a later re-transcription quietly taking the shield hand back, and carried in the source-diff verifier as a **named allowlist entry** rather than a blanket exemption — a fifth disagreement with `skills.c` still fails it | Done. One consequence worth knowing rather than fixing: the house 95 cap already levels paladin with warrior in 1h, as it does in 2h — the source's 95-vs-100 gap does not survive `SKILL_CEILING`, which is the cap's doing and not this row's. Reopen it only if the cap itself is reopened |
| **A bashed body should not get away, and a bash should interrupt a cast** (owner, 2026-08-08, seventh ask) | **Both already true, and each arrived by a different route than the ask supposes — which is why the row is worth keeping rather than closing silently.** *Fleeing:* the source does **not** pin a floored body — `CMD_Y(CMD_FLEE, STAT_NORMAL + POS_PRONE, …)` (`interp.c:2717`) sets flee's minimum position at `POS_PRONE`, which is `0`, the bottom of the scale. What stops the escape is that the attempt is **spent**: `flee.ts:174` puts a non-standing body on its feet and returns `scrambled` without moving it, transcribing `do_flee`'s *"Looking panicked, $n scrambles madly to $s feet!"*. So a bashed mob is one round behind rather than held, and the ask's intent is already the behaviour. *Casting:* `bash()` carries no `StopCasting` at all — the source interrupts from `do_garrote` (`actoff.c:10615`) and a face-strike (`fight.c:6866`), never from a bash — but ours interrupts anyway, and by a better seam than a call inside the verb: the wind-up re-validates once a second, and `index.ts:4174` ends the cast with *"you have lost your footing"* the moment posture leaves standing, which is precisely what the knockdown at `index.ts:3528` does. One rule catches bash, every future knockdown, and every forced exit | Nothing to build. Recorded so the question is settled rather than re-asked |
| **Bash should be class-specific — a mage should not have it** (owner, 2026-08-08, sixth ask) | **Already true of the mage, and the ask caught a real gap one group over.** The refusal exists: `ceilingFor(…) === 0` turns the verb away before it costs anything (`index.ts:3464`, *"Your training never covered bash"*), and `wizard` and `rogue` both carry `bash: 0` — Phase 21 slice 4, whose own comment is *"a wizard does not bash badly, a wizard does not bash"*. But the source's table is **narrower than ours**: `skills.c:3727` grants bash to **paladin, antipaladin and warrior only**, each at level 1 with `maxlearn` 100, and to no one else — the whole grant, since `SKILL_BASH` appears exactly once in `skills.c`. Ours is keyed on the four class *groups*, so `priest: { bash: 60 }` lets a cleric, a druid and a shaman bash when no priest class in the source has it, and the warrior group's unwritten default of 95 does the same for a **ranger**. **Four of our nine classes can bash and should not: ranger, cleric, druid, shaman.** | ✅ **Built the same evening, and the owner took the whole re-key rather than the one-line patch** — which was right, because the priest row was a symptom: the fold grants a skill to *every* class in the group, so the same bug was sitting under every row of the table. `CLASS_SKILLS` now carries `skills.c`'s own `{ level, max }` per class, absent meaning 0; the 95 became a **cap** in `ceilingFor` rather than a default, so the table stores the source's number and can be diffed against it; specialisations are excluded, which removed the zealot's dual wield that slice 4 had folded into a flat priest 40; and dodge is adopted from inside the comment block that `skills.c:3968` wraps its whole class list in. Three level gates now bite — warrior reach 25, ranger rescue 10, rogue parry 20 — so `ceilingFor` takes an optional level. Two faithful transcriptions worth naming because they read as bugs: a **paladin has no 1h weapon skill** in the source (*overruled the same evening — see the row above*), and a **rogue has no 1h slashing** (commented out, with the shortsword exception hardcoded in `fight.c`). `ClassGroup` is deleted. Checked by re-deriving all 144 class/skill pairs from `skills.c`: no discrepancies |
| **Bash should refuse without a shield** (owner, 2026-08-08, fifth ask) | **The source says the opposite, and the owner has chosen the source's model over the ask.** `bash()` (`actoff.c:6132`) never gates on a shield: without one it multiplies the chance by `MAX(20, SKILL_SHIELDLESS_BASH) / 100` and says *"Bashing without a shield is tough, but you try anyway..."*; with one it **adds** `weight / 1.8` to the chance and `number(0,4) + weight/2` to the damage, doubled when the shield's name contains *"spiked"*. The hard gate the ask describes is a real rule, but it belongs to **`shieldpunch`** — `if (!ch->equipment[WEAR_SHIELD]) { send_to_char("Punch with what?\n", ch); return; }` (`actoff.c:7666`) — which we have not built, and which is easy to conflate with bash. What makes the ask nearly right is that `shieldless bash` is **epic**: `SKILL_CREATE("shieldless bash", SKILL_SHIELDLESS_BASH, TAR_PHYS \| TAR_EPIC)` (`epic_skills.c:172`), so for every non-epic character the multiplier is floored at 20 **permanently** — a shieldless bash is a fifth as likely to land, for good, and the shipped game therefore *feels* like the ask even though it never refuses. Ruling: transcribe the graded penalty, not a refusal | ✅ **Built 2026-08-10, the ruling taken literally.** `CombatAbility` grew the worn-gear signature the row predicted the next ability would want — `abilityChance(ability, learned, shield?)` and `abilityDamage(…)`, with a `usesShield` flag on the table so the next one says so in data rather than in an `if`. Bare-armed, a bash lands at **a fifth of the chance** and says so first (*"Bashing without a shield is tough, but you try anyway..."*, printed before the roll as `actoff.c:6299` does, because it is a warning about the attempt and not an excuse for the miss). With a shield it gains `weight / 1.8` on the chance and `number(0,4) + weight / 2` on the damage, **doubled when the name contains "spiked"** — the source's own substring test, which 19 of our shields answer. Two things the build had to settle. **We have no `weight`**: pounds became `sizeFrom`'s slots of bulk deliberately, so rather than re-introduce a second unit the term is rebuilt from the midpoint of the size bucket, ±1.4 points off the source's curve — and it is worth stating that this is not a rounding term, because the catalogue's **242 shields span 1 to 50 lb**, making the bonus worth +0.6 to +26.7 points. And **the starter kit has no templates**, so testing `DURIS_ITEM.shield` alone would have silently excluded every new paladin and cleric — the exact characters who bash — which is why `shieldInHand` also knows `STARTER_SHIELD_ID`. Epic skills still do not exist; `SHIELDLESS_BASH_FLOOR` is the named constant that becomes a lookup on the day they do. Needs `CombatAbility` to read worn gear — a signature the next ability will want too — and somewhere for epic skills to live. Note we have no `shield` wear slot (a shield is `offHand`, identified by `ITEM_TYPES.shield`), and `STARTER_KIT` fills `mainHand` only, so a hard gate would have made bash unusable from level 1 — the graded penalty does not. Sits beside the DEX/STR scaling `abilities.ts` already names as waiting on ability scores |
| **Make the Faerie Realm walkable from the kobold zone** (owner, 2026-08-08) | **Agreed and built, and the measurement changed the question.** It could not be done by loading zones: across all 327 harvested zones the Faerie Realm sits in a **six-zone island** — 64, 190, 193, 226, 367, 423 — with **no walking route to the mainland's 268 from anywhere**, so there was no chain to switch on. Our geometry comes from a player-made zMUD map, which records routes somebody walked; in the MUD the Feywild was evidently reached by magic. The measurement also found the worse fact underneath: the **loaded** world was six disconnected pieces and the kobold settlement touched none of them, so a new character's whole reachable world was 99 rooms. Fixed with the two things the data did support. **321 Evermeet- Ancient Forest** was loaded — 100 rooms that already border the settlement in the harvest (41187 south to 41185, both ways), its only harvested neighbour. And the crossing was **authored**, which needed a new overlay: `data/world/overrides/links.json` and `server/src/links.ts`, deliberately separate from `rooms.json` because that file refuses exits by name and should keep doing so — *"those are geometry."* Three rules earn it: **a link is a pair and both halves live in one record** (the failure is a door you cannot come back through), **it may never overwrite a harvested exit** (silently re-routing a door is a zone-shape change invisible in this file), and **every authored link is a `portal`** — not a choice, a fact, since two rooms in different zones share no coordinate space, and `RoomExit.portal` already existed and is already drawn as a portal rather than an opening. Applied as a pass **after** every zone is indexed, because a link's two ends are in two zones by definition. Owner also chose to load the **whole faerie cluster**, 906 rooms. **The honest gap: only zone 64 has harvested population**, so Leuthilspar's forest and the Unseelie Court are 471 rooms of fully-described, empty world | ✅ **Built 2026-08-08** — 11 tests; the faerie ring runs from a room whose prose was authored to announce it to **7691 "A Ring of Rowan Trees"**, which the map had already named and which already drops into Finn's Keep. Walked **17 rooms from the spawn point** by a client discovering the route from the exits alone. The overlay is also **Phase 22's cross-source exit rule arriving early** — Velen's three roads need exactly this |
| **The armour was too wide — quest givers should be killable unless we say otherwise** (owner, 2026-08-08, correcting the same day's fourth ask) | **Agreed and corrected within the hour, and the correction is more interesting than the original.** Verbatim: *"it appears when I made it earlier that quest mobs can't be attacked or damaged I may have made a mistake. the viscount for example should be killable so we need to add the can't kill/damage flag to quest mobs that we don't want to die and leave it off when it doesn't matter if they die."* The morning's build hung **two facts on one bit**: `EntityView.questGiver` marked the asker *and* seeded `combat.ts`'s untouchable registry, so offering work made a body immortal and there was no way to express *asks for help, and can be murdered for it* — which is most givers, and is also a legitimate player choice the blanket rule quietly removed. Split into two: **`QuestDef.protectGiver`**, absent by default, seeds the registry; `questGiver` keeps only the badge. **Protocol 27** adds `EntityView.untouchable` beside it, because a menu must never offer a blow the server will refuse — so a killable giver's click menu now shows *Quest* **and** the openers **and** *Attack*, and an armoured one shows only *Quest*. Deliberately **a property of the body, not of quest-ness**: nothing in `untouchable` mentions quests, which is what lets a shopkeeper or a herald carry it later without being given a quest to justify it. The flag is **OR-ed across a giver's rows** (a body cannot be half-immortal, and last-row-wins would make the armour depend on file order), it round-trips through `saveQuests` (a writer that dropped it would turn fixing a typo in the *ask* into a giver anybody can kill — tested), and it is a tick-box in the admin editor that re-arms the live world with no restart. **All eight shipped quests were left unflagged**, which is the owner's own default: the Viscount, Gwark, Finn and the rest are ordinary bodies again | Phase 21 slice 7 amended the same day ✅ — driven on the wire in all three directions: unflagged giver takes a blow, ticked box refuses it *"has no quarrel with you"* while still offering work, unticked box takes a blow again |
| **A capital city — walled districts stitched from many zones, an underworld beneath, patrolled walls above, guild training, an ocean dock, roads out north/east/south, roaming townsfolk, a mages' tower, inns, a square noticeboard for the gods, shops of every kind, dangerous quarters, rare loads; a bank if coin gains weight; weather and wind last** (owner, 2026-08-08, the evening ask) | **Agreed, and it is big enough to be the schedule's next act rather than a row.** Interviewed the same evening, three decisions closed: the name is **Velen** (owner's pick from the shortlist — not Waterdeep, not any city with a game named after it — reserved in `names.ts` the same commit), the geometry is **authored, not harvested** (`data/authored/`, committed, ids 900001+ on Windsong's convention — the first content that is *ours* and shippable), and the first milestone is **the town square, alive** — board, inn, shops, gates — growing outward. §4 question 2 splits the ask cleanly: five smallish mechanisms (authored-zone pipeline with one cross-source validation rule, boards from `boards.c`, trainers from `guild.c` finally moving skills stored at 0, patrol/wander from `specs.mobile.c`/`mobact.c`, rare-load spawn chance), everything else content that spends the first six acts. Two pieces parked inside the act on the owner's own words: the bank behind the coin-weight decision (*"we may need a bank if we start giving the currencies weight"* — the Underquay vault gets built and locked either way), and weather-and-wind explicitly after everything else (*"this can wait until everything else is done"*) | **Act VII, Phases 22–27** — [DESIGN-city.md](DESIGN-city.md), written the same evening |
...
| **Either-hand weapons — light blades wieldable in main OR off hand, Windsong among them** (owner, 2026-08-07) | **Agreed, and it is the doorway to dual-wield, which decides its size.** The *flag* is small — a `handedness: 'either'` beside `twoHanded`, a `wield <weapon> offhand` form, the paper doll already has the slot — but an offhand weapon that never swings is a stat stick wearing a sword's name: the thing that makes it *matter* is the **offhand attack**, which is a combat-round change (the source rolls the second attack off `SKILL_DUAL_WIELD` with its own to-hit penalty, and our round grants one swing per actor by design). So the flag, the wield form and the Windsong/dagger data land **with** the dual-wield skill, as one visible slice — not before it, where the flag would sit tested-and-meaningless, which is the four-mechanisms failure `ROADMAP.md` rule 1 exists to prevent | **✅ Built 2026-08-08, as one slice exactly as the row asked** — and the research overturned this row's own premise about the penalty. The row says the source *"rolls the second attack off `SKILL_DUAL_WIELD` with its own to-hit penalty"*; it does not. In the **live** branch the off-hand call is `hit(ch, opponent, ch->equipment[WIELD2], …)` (`new_combat.c:2343`), byte-identical to the main hand's but for the weapon argument — **no to-hit penalty, no damage penalty, nothing**. The gate *is* the price: a skill-rolled chance to swing at all (`skill > number(1, 101)`, so `(skill − 1) / 101`), plus a wield-time restriction to light blades, so the second die is small. `PhasedAttack` — what `:2340` actually calls — is declared at `prototypes.h:950` and **defined nowhere in the tree**, so the inline form its own commented-out predecessor and its two live haste/blur siblings all spell is the only version of the rule the source contains. Shipped: `dual-wield` as a `SkillId` with the source's class table folded onto our four temperaments (warrior 95, rogue 80, priest 40, **wizard 0 — no mage class appears in `skills.c`'s list at all**, so the 0 is the refusal `wield … offhand` reads) — *re-keyed per class later the same evening, which moved the rogue to their own 75 and the priests to 0, the flat 40 having been the fold inventing a grant out of `SPEC_ZEALOT`*; `handedness: 'either'` on `ItemTemplate` and `Item`, derived at `instantiate` from Duris' own `IS_DIRK` (dagger + short sword, one-handed) under a bulk ceiling of 3 slots derived from `actobj.c:4918`'s strength gate, and **authorable** in both the authored-item and override paths so Windsong — a `weaponClass` 5 scimitar the automatic rule rightly declines — is blessed by hand; the second blow inside `advanceCombat`'s existing round through the same `swing`/`landBlow` seam, so there is no second damage path; `wield <weapon> offhand` as a **suffix on the existing verb**, so `commands.ts` gains no row and the abbreviation table is untouched. Drive: two daggers, both blows 0 ms apart inside a 3,200 ms round, the off-hand line marked `(off hand)` in cyan, and `dual wield 40 → 41` notching off `notch_skill(…, 17)`. Left for later, named: the NPC branch (`GET_LEVEL(ch) * 2` — no harvested mob carries two weapons yet), and Duris' four-handed thri-kreen `WIELD3`/`WIELD4` |
| **Opening moves on the mob click menu — bash, backstab, cast with a spell submenu** (owner, 2026-08-07) | **Agreed, and the shape exists in the codebase three separate times already — this row is assembly, not invention.** The asks, verbatim: *"if a player has opening moves when you click on a mob it should be part of the menu. for example if you can start with a bash (and are wearing a shield as you can't bash without one) the menu should include bash"*; *"casters should see cast as an option then be presented with another menu that lets them select the spell"*; *"rogues get backstab as another example"*; and typed stays primary — *"if they are typing in the left panel then it is just cast spellname mobname or bash mobname"*, which already works today. Three pieces, each with a precedent. **(1) A menu row is protocol 11's rule again** — a click may not be more powerful than a word — so a *Bash* row is `useAbility` fed by entity id, exactly as the `attack` intent already routes `kill`. **(2) Which rows appear is a capability question only the server may answer**: bash is class + shield + the ability's own gates, and a client-side re-derivation would drift from the server's refusals the first time a rule moved — so the menu wants a small "what can I open with" answer riding `SelfView`, listing ability ids and, for cast, the same levelled list `/spells` already prints. **(3) The cast submenu is `/spells` pointed at a menu.** **Backstab is the honest blocker**: `SKILL_BACKSTAB` is not built (no positioning rule, no multiplier ladder), and the menu must never offer what the engine cannot do — the rogue's row lands with the skill. Race-gated openers ride the same capability answer once Phase 21's innates exist | Lands with Phase 21's classes, when "what can this character open with" has more than one answer; the capability message and the bash/cast rows are small once that phase names the abilities |
| **`quaff`/`drink` for potions, `read` for notes, `eat` for buff foods** (owner, 2026-08-07) | **Agreed, and they are three different sizes.** **`quaff` ✅ built 2026-08-07, the same night**: the scroll layout, the scroll reader, 662 potions harvested (150 castable) — plus the three drinking rules that make it its own verb: everything on the drinker, one draught per thirty-second timer, and a flat-50% mid-fight spill (`CMD_Y` where recite is `CMD_N`). **`read` ✅ built 2026-08-07, the same night** — the measurement the verdict asked for said yes: 14,936 real object `E` blocks are 2.8 MB onto a **server-only** file (10,591 templates now carry prose), and the 3,960 room blocks ride the spawns' own name join behind the `--descriptions` switch (639 attach across the 49 matched zones). In the source `do_read` is one line — `do_look("at <arg>")` — so the slice was the harvest plus `new_look` case 7's search order (room sign, worn, bag, ground; exact-word `isname` matching; the two distinct refusals). Boards are still their own machine (`boards.c`) and wait. **`eat` ✅ built 2026-08-07, same night** — and the guess about its shape was wrong in the best way: the source's foods do not cast spells, they grant **regeneration** (`value[1] × 15` hp/min for `1 + value[0]` ticks), which IS the owner's fast-heal memory, riding second wind's own apply locations. 541 harvested, 36 poisoned traps kept, hunger still explicitly off. The drive also flushed a latent affect-stacking bug that had been eating bless's hit node since slice 5 | All three ✅ built the same night they were asked; boards are the one deferred piece |
...
| **Rename IceCrag Castle to Tordraken Castle, re-themed to match** (owner, 2026-08-09) ✅ **built 2026-08-10** | **Agreed, and it is a rewrite wearing a rename’s clothes — the owner’s own words: *“so it will need a rewrite.”*** The lore arrived with the ask, from the same map that named the city: **the Castle of Tordraken was the ducal keep of the Duchy of Draknor and later the Cape Velen duchy, built atop a tall sea cliff, with ballistae and catapult batteries installed against naval assault** — and Tordraken sits on the Dragon’s Head just northwest of Velen, so the castle becomes the North Road’s landmark and the duchy assembles around the capital. What makes it a rewrite: zone 36 is Duris-matched — **217 of 219 rooms carry harvested prose describing an ice fortress**, and a sea-cliff ducal keep cannot be made of it by find-and-replace. The room names and prose re-author through the A5 overlay that already survives worldgen; the population (levels 15–60) likely re-flavours from frost things to garrison things through A9’s mob overlay; the batteries are furniture and prose until siege is ever a mechanism. **Carries: a zone-name override for harvested zones** — the one field no overlay can rewrite today (rooms.json stops at rooms; zones-authored.json names only authored zones), small and honest: a name field keyed by zone id, composed at boot exactly as room overrides are | **Act VII, beside Phase 27** — the roads phase reaches the wider duchy, and renaming the castle the North Road points at belongs to the same pass. The zone-name override can land earlier as a small A row if wanted |
| **The graphics upgrade: an open world drawn from our grid maps** (owner, 2026-08-09) | **Agreed, designed the same day — [DESIGN-open-world.md](DESIGN-open-world.md) is the plan.** The owner’s sentence contains the whole architecture: *“we use our maps for the layouts, and if we need to keep people on paths etc we can do that with trees, fences, rivers, buildings”* — the room graph stays the law of movement, and the void between room blocks becomes diegetic blockers on no-exit borders. A presentation change wearing a worldgen change’s clothes: rooms already carry grid positions (the zMUD map’s own layout), so seamless rendering is a re-projection of data we hold, not new data. The safety story is a validator that flood-fills the rendered tilemap and asserts reachability equals the room graph, both directions — the pretty world provably still the MUD’s world. One genuine risk owned in the doc: open sightlines versus room-and-neighbours interest management (§4’s three dials, camera bounds first). Sliced V8a–V8d: **V8a built 2026-08-09** — Velen Square fuses (SEAM_GAP projection, pair-stamped seams, the flood-fill validator in tilemap.test.ts), then the blocker law with its validator, then the harvested wilderness, then transitions and dressing. **V8b and V8c built 2026-08-10**, and an owner ruling mid-slice set the law the arc now runs on: *“portals should only be used to traverse large distances or for magic”* — so a border is a road, and the ring is the exception that must be earned. V8b fused Velen's three levels into one Place and taught the seamless projection to lay a wall across a door before stamping its gate; V8c gave `RoomExit` a `seam` flag, made worldgen mark **every horizontal portal** one (cross-zone and in-zone alike — an in-zone portal is the mapper's layout giving up, not a teleport), and moved the crossing into `sim.tick` so pressing against a room's edge simply walks you through. **5,328 of 6,453 portals stopped announcing themselves; visible rings in the loaded world fell 109 → 21** — eight staircases, eleven clambers the mapper drew flat, two pointing at unloaded zones. Two camera fixes fell out of it, both the same bug at different scales: a jump larger than a room is a teleport, not drift, so the view snaps rather than lerping the map past a player who has already stopped. **V8d's scenery built 2026-08-10**, answering the owner's *"a fountain or pothole or something like that, just for atmosphere"*: a `scenery` field on rooms, a six-prop catalogue in `shared`, and ten things now standing in Velen including the fountain and plinth the Great Crossing's prose has promised since Phase 23. The rule that made it small is **a prop is a thing in the way** - paint stays a coordinate hash in the client where it cannot desync, and anything that occupies ground is a rule both sides stamp from one table (protocol 31). Two findings: `Tile.Prop` had to be a *second* solid kind because solidity and opacity are different questions (you walk round a fountain and see straight over it), and reading the opaque one to write it turned up that **`Blocker` was never in `isOpaqueTile` at all** - every wall the seamless projection had stamped since V8b was see-through, against its own docblock and a test comment that both claimed otherwise. The rest of V8d - transitions, wilderness scatter, rivers and bridges, the city as streets between buildings - is still open | **Track V, as the V8 arc** — the owner chooses its order against Phase 23 (doc §6.4): seamless-then-content walks Velen’s new face once |
| **Real item-icon art for potions, scrolls, wands** (owner, 2026-08-10) | **Found from the shop floor: a bought healing potion rendered as a shield.** Measured: the art index holds 347 entries and every one is worn gear or a weapon — no flask, bottle or vial exists — so the A7g sweep, forced to choose, dressed **364 potions in heraldry** (146 in shield-round-brown alone, plus one in moon earrings and two in black sashes). All cleared; potions now fall to the honest cyan flask category shape. The real fix is a **second art channel**: item *icons*, distinct from worn *layers* — the packs already hold the pixels (`decorations-medieval.png` and the interior sheets in assets/love2d-lpc-tiles carry bottles and flasks), and the artgen index needs a kind that says "this draws in a bag, never on a body" so the sweep can never again reach for a shield because the wardrobe held nothing else | **Track V, beside A7d’s icon work** — small once the icon-kind exists; the sweep re-run over type 10/15/16 finishes it |
| **V8e - the world on one plane** (found 2026-08-10, answering the owner's *"is there still supposed to be a portal?"*) | **The measurement overturns a Phase 1 belief.** Every doc in the project says two Places share no coordinate space and 0 of 991 cross-zone exits is a geometric neighbour. True of the emitted data; false of the source.  computes minX/minY **inside a per-zone loop**, and that normalisation is what destroys cross-zone adjacency - the mapper's raw ObjectTbl coordinates are one global canvas. Read that way: **111,765 of 116,699 within-zone exits (95.8%) and 547 of 1,006 cross-zone exits (54%)** are true neighbours, and among the zones we actually load it is **17 of 19** - including all thirteen between the Stag Forest and the Stump Bog, the kobold settlement's border with Evermeet, and the Faerie Realm's with Leuthilspar and the Unseelie Court. The work: worldgen records each zone's global origin (the number it already computes and discards), and  learns to build one grid from several zones by placing each at its offset - the same merge Velen's districts got by hand in V8b, generalised and driven by data. The remaining 46% scatter across 357 distinct offsets and stay portals honestly. See DESIGN-open-world.md section 5b | **Track V, after V8d** - it is a  change (about a hundred call sites) and wants its own slice and its own tests rather than riding on a rendering change |
...
| **Scenery you can use - drink at the well and the fountain** (owner, 2026-08-10) | **The mechanism exists in the source's shape; the *reason* to use it deliberately does not.** `do_drink` (`actobj.c:3026`) refuses anything that is not `ITEM_DRINKCON` and reaches into the room's contents as readily as your hands - so in Duris a fountain was **an object standing in a room**, never a room feature. Our own catalogue proves it: of the 34 items named *fountain* or *well*, exactly one - *a small fountain* - is a drink container, out of 269 in all. That settles the shape: a prop should be **the picture of something the room already holds**, not a new interactive system bolted to the scenery table. What it cannot buy is an effect. Thirst is condition 2 of Duris' five (`defines.h:633`), and this file's own *Explicitly not scheduled* line records that Duris built hunger, thirst and aging, shipped them, and switched them off. **Drinking with no thirst is flavour** - worth having and cheap, and the danger is that it drags a condition system in behind it, which is the thing to refuse | **Track V, small, blocked on nothing.** Question 2 says content: Phase 15's items and Phase 23's rooms being *used*, not a new mechanism. Ships with the row below, which is the same idea |
| **The plinth should be the noticeboard you read** (owner, 2026-08-10) | **Already half true, and the missing half is a name.** The Great Crossing has carried `board: velen-square` since Phase 23 - `read board` lists its messages and `read <n>` prints one - and V8d then stood a plinth in the same room because the prose promised one. **Two facts about one object, authored twice and joined by nothing**, which is exactly the seam a player finds by typing `read plinth` and being told there is no such thing. The fix is the general rule all three of these rows are reaching for: *a prop is the visible form of something the room already has*. So `read plinth` and `read noticeboard` resolve to the room's board, and better still the prop is **derived from `board`** rather than placed beside it - one authored fact, one picture, and no way to have a noticeboard nobody can see | ✅ **Built 2026-08-10, and it found a bigger seam than the one it was aimed at.** `look` consulted neither extras nor scenery, so `read fountain` answered while `look fountain` said *"You do not see that here"* about a fountain filling the middle of the screen - `do_look` (`actinf.c:2632`) walks room extras, then equipment, then objects, through the same `find_ex_description` our `read` already used, and ours walked none of them. Fixed with the plinth: every prop gained keywords and a description, matched whole-word and case-blind, with authored `extras` outranking the catalogue so a room that wrote its own fountain keeps its own words |
| **Search the haystack, with a chance of a needle** (owner, 2026-08-10) | **The only one of the three that is a real mechanism, and it earns its place by closing two asks rather than one.** `do_search` (`actobj.c:5771`) is a room-or-object verb gated on `find_chance(ch)`, and we have no `search` at all. The row already in this lot - *"Hidden items in corpses, found by searching"* (owner, 2026-08-06) - is the same verb wanting the same gate, so one build answers both; question 2's test is whether anything existing can hold the idea, and here nothing can. The needle is **content that follows the verb**, not part of it, and the quest system to hang it from has existed since A7q. Worth noting the owner said *"part of a quest in the future"* - so the verb is the ask and the needle is the illustration | ✅ **Built 2026-08-10 — the verb, and the needle. The corpse row is *not* closed and that is worth saying.** `search` is transcribed whole: no argument searches the room, a named container or corpse searches its contents, and a named prop searches the room around it (our extension — Duris has no scenery, but its own second case is *a thing standing in the room that has an inside*). One find per search and a round charged whether or not anything turns up, both the source's. **Three of `do_search`'s four finds have nothing to find here** — no traps, no `EX_SECRET` in the harvest, no hide — and a fourth refusal is unreachable because our `ContainerRule` has no `closed`; all four are named in `search.ts` rather than shipped as dead branches. Two conversions were forced. **Duris' stats are percentile** — `STAT_INDEX` (`utility.c:4205`) buckets in steps of six across `str_app[52]`, and `find_chance` rolls the raw average against `number(1,101)` — so copying the comparison onto our SRD 3–18 would make the best searcher in the game succeed 18 times in 100; `SCORE_TO_PERCENT` is the ×5 that fixes it. And **luck is dropped**: `GET_C_LUK` is Duris' seventh stat and `ABILITIES` is the SRD's six, so the average is over two. The needle rides a new `RoomScenery.conceals`, deliberately on the *prop* rather than in a reset's object list — a reset would put a needle in the same room, which is a different sentence from the bale being what it is lost in. **What is still open: nothing can mark a corpse's loot hidden**, so `search corpse` runs and can never succeed. `AuthoredLoot` needs a `hidden` flag, which is a small follow-up and the actual close of the 2026-08-06 row | ✅ **Closed 2026-08-11 by `AuthoredLoot.hidden`.** The flag threads carried loot into a corpse unlisted, `lootCorpse` refuses to hand it over, and `search <corpse>` is the only way it comes out. Two things the wiring turned up: **a corpse is not a ground item** — its own store, reach rule and name matching, all of which `loot` already owned — so `search sentry` had to go through the same three or the two verbs would disagree about which of three dead guards they meant; and **the body stays drawn as a pile** while it holds one, because `looted` is derived from `contents.length`, which turns out to be the only clue a player gets that there is more in there. `hidden` is **carried-only** — worn gear is visibly worn, so a hidden helmet would be a lie the moment anybody looked at the wearer — and it composes with `percent`, which is the shape a real secret wants. A warden's tally rides the brigand that took the roadwarden's cloak. **The owner's litter question (2026-08-11) was already answered by the shape**: a rotting corpse spills its contents with the ordinary ground clock and the flag rides along on the item, so a hidden thing lands *still hidden* on the floor, stays findable by `search` for as long as any dropped thing lasts, and is then collected like anything else — the owner's own second option, and nothing accumulates. Pinned with a test rather than left to fall out. Next: the quest the needle is for |
| **The panel needs a section for posting notices to the plinth** (owner, 2026-08-10) | **Already built, and worth a line here so it is not asked a third time.** Phase 23 shipped the whole path: `GET /admin/api/boards` lists every board with the rooms it stands in, `POST /admin/api/boards/<id>/posts` pins one up, `DELETE .../posts/<n>` takes it down, and the **Messaging** section drives all three - board picker, *signed as*, headline, body, a *Pin it up* button and a newest-first list with *take down* beside each. Posts land in the same store `read` serves and persist to `data/world/overrides/boards.json`, so the panel and the square can never tell a different story. Verified end to end the day the plinth landed: a notice pinned through the API came straight back out of the board listing and onto disk. **What was missing was never the panel - it was that the plinth and the board were two authored facts about one object**, which the row above fixes | Nothing to build. The pairing is the point: A2's Messaging section is the *only* way a notice gets written, since no player-facing `write` verb exists |
| **Pivot to the 3D client now, before more LPC art is bought** (owner, 2026-08-11) | **Agreed, and it reverses a standing ruling — which is why it is written down rather than just done.** The roadmap has said since 2026-08-07, on the owner's own re-affirmation, *"think about it, build none of it yet"*, on the reasoning that everything being built was the proof of concept that the mechanics survive the port. **That proof is substantially delivered**: combat, skills, spells, quests, shops, accounts, seams, scenery and search all exist and all live in `shared`/`server`, where no pixel does. The trigger was the owner declining to fetch an animal pack — *"before we get all hung up on getting LPC images that aren't going to be used in the end anyway"* — which is the plan's own §2 conclusion reached independently: *"the LPC investment is a write-off. Not partially — entirely."* **What is not a write-off**, and is worth stating so nobody re-does it: the *decisions* are renderer-agnostic. `mobpick` classifies a creature from its name, which is exactly what selects a 3D model; `scenery.ts` says what stands where, that it is solid, and how scatter is derived deterministically. Only the sheet-staging half of each is LPC-specific. **One correction to the plan before anybody costs it**: §2 prices the port against a 539-line `scene.ts`. It is **5,330 lines** today. The salvage list is still right; the effort table under it is not | **Track V, and it becomes the track. M0 and M1 both built 2026-08-12, by delegated agents, verified independently.** M0: sparse 16x16 chunk storage (zone 317: 521 MB dense -> 214 KB, x2,553), the sector-0-void fix, stair metadata kept, outdoor rooms merging along their whole shared edge, and `space.ts` with the mirrored-world test. M1's big finding: **the plan's own narrative was stale** - the baseline already had suffix rules, graph diffusion and a Duris sector harvest, and the world was already at 0.2% defaulted. The real repairs: landscape words now outrank road/city (444 distinct names like "A Dark Forest Path" were roads; **4,427 rooms - 9.5% of the world - changed sector**), and the weak zone-name tier stopped being a protected diffusion seed after a held-out validation against the harvest scored the graph at 65-69% where the zone guess managed 48%. **M2, M3, M4 and pointer input all landed 2026-08-12** - the IR with its seams-are-not-portals ruling, the grey-box client beside the Phaser one, the night recipe with Neutral-vs-AgX switchable, and the mouse. **GO — the owner's verdict, 2026-08-13, judged in the world against the reference.** The bet held: the night reads as night. M5+ unlocked. Started at the plan's M0 and M1 because they need no Three.js and no new dependency: sparse `tilemap.ts` chunks (the current whole-zone render texture cannot open zone 317) and repairing `inferSector`, which first-match-wins a regex and defaults to `field`. Both are worth doing whatever the renderer is, which is what makes them the right first step |
| **A winding road should actually wind - open-world variance over the grid, using our maps** (owner, 2026-08-13) | **Agreed, and the plan named this exact criticism in section 4 before any 3D existed** - *"a lattice of 9m squares joined by 3m necks, repeating on a hard 12m beat."* The answer is a **deterministic domain warp**: a smooth seeded low-frequency displacement field over world position, applied to everything visual - ground, room centres, walls, scatter, and entities as rendered - so a chain of east-east-east road rooms draws as a road that drifts and bends. **Collision honesty survives by construction** because walls render through the same lens as floors: what you see is still exactly where you can walk. Amplitude scales by sector (zero for inside/city so buildings stay square; full for road/field/forest/swamp/hills/desert), ramped at boundaries via the IR's existing blend weights; bounded gradient so the warp never folds. The simulation grid, the server, and the MUD's room semantics are untouched - the warp is a lens, not a move. **Explicitly rejected**: re-embedding the graph with varied edge lengths/angles - weeks of collision-layer surgery that invents geometry the mapper never drew, where the warp bends the map we actually have | **M5c, immediately after M5a lands** - it touches the same files M5a is editing now (ground mesh, scatter placement, chunkPlan), so it queues behind M5a exactly as pointer input queued behind M4 — and M5b then jumped ahead of it on the owner's 2026-08-13 confirmation of the kit look: the warp displaces kit instances exactly as it displaces grey boxes, so bending the roads loses nothing by coming after the world gets its clothes |
| **"I love it — lets work with that and build my game in a world that looks like that"** (owner, 2026-08-13, eyes on the Stylized Nature MegaKit) | **The §5 art direction is now a ruling, not a proposal — and acquisition day paid for itself in retired risk.** Six packs are on disk (723 MB, CC0, git-ignored under `assets/quaternius/` with `PROVENANCE.md`): Nature MegaKit, Medieval Village, Base Characters, Fantasy Outfits, and Animation Libraries 1 *and* 2 — the itch flow mints signed URLs that expire in **60 seconds**, so mint and curl must share one window. Standard-tier truths measured on disk rather than read off listings: the Nature kit's GLTF line is **68 models** in exactly the families the world needs (CommonTree/Pine/DeadTree/TwistedTree ×5 each, bushes, four grasses, flowers, mushrooms, pebbles, rocks, and rock-path pieces for road edges); AnimLib 2 is 43 clips — rich sword set, block, knockback — but **no plain walk, run or death**, which is why UAL1 joined the same hour (15.9 MB: `Walk_Loop`, `Jog_Fwd_Loop`, `Sprint_Loop`, `Death01`, `Hit_Chest`/`Hit_Head`, and a `Spell_Simple_*` set the caster classes will want); together they cover M7's whole state machine. **The plan's one unverifiable M7 risk died today, without Blender**: GLTF is JSON, so the armatures were diffed headlessly — every rigged file in all four character/animation packs binds within the **same 65 joints by name**, zero violations across 39 files. No retargeting project hides inside M7. *Ultimate Monsters* is not on itch ("invalid game" — likely Patreon-only): monster models stay an open M7 question | **M5b, promoted ahead of M5c** — the owner asked to see the world in its clothes: kit import through the gltf-transform pipeline, water, wetness, and a daylight recipe (M4 tuned only night; the kit's own look is sunlit). CLAUDE.md's art rule now covers both clients — LPC frozen for Phaser, Quaternius for the game's future |
| **"Are we going to be able to zoom out?"** (owner, 2026-08-13, first minutes in the dressed world) | **Yes — a clamped dolly, and M5b accidentally did the hard part.** The camera is fixed at 36 m / 30° fov / 64° pitch by design (M3), and three systems are tuned to that exact frame: the streaming ring, the per-frame shadow refit, and — since today — the undergrowth fade bands, which M5b *derives from the camera constants in a test* after finding that a hand-authored band had silently erased every grass tuft in the world. That derivation is exactly what makes zoom cheap now: turn the camera distance into a clamped variable (roughly 24–48 m), let the fade bands and the shadow fit follow it by the same derivation, widen the streaming ring one cell at the far stop, and the wheel is a zoom. Fog of war already answers for the extra ground it reveals | **Queued immediately behind M5c** — zoom touches the same camera/pointer/unproject files the warp is editing at this moment, so it waits exactly as M5b waited behind M5a. ✅ Built the same day (688fd11) — live-tunable; the baked defaults await the owner's pick |
| **"How long is this rain going to last?"** (owner, 2026-08-13) | **Until you press R — and that answer is the finding.** Rain is M4's visual toggle, not weather: nothing in the world decides when it starts or stops, exactly as nothing ticks a game hour (verified twice today — the only "hour" under `server/src` is scheduler timestamps and a food-duration unit). The real answer is the MUD's own: Duris `weather.c` drives sky states and per-zone climate off the game clock — pressure, fronts, sector-aware messaging — and transcribing it gives weather that belongs to this world rather than a particle switch. It pairs with the game-hour clock as one **world-clock server slice**: hours drive the daylight recipe (M5b's `setGameHour` is the named hook, dawn and dusk already scrub with `[` and `]`), weather drives rain, wetness and fog through the same recipes, and the 2D client's text log gets the sky messages for free | **One server slice, after the M5 track closes.** Every client hook it needs already exists and waits: `setGameHour`, the `uWet` ramp (6 s wet / 40 s dry), the roofed rain gate. Nothing client-side blocks it. ✅ Clock and weather built the same day (f85b5fb) — and the transcription found Duris scrapped the Diku sky machine in 1991 for a continuous climate model, so there was never a "rainy" flag to read, only pressure and temperature; the 3D wiring followed the same evening; snow-vs-rain particles were ruled the same night — *"draw real snow"* — and built: alpha-blended shader discs (additive would smear on a bright noon sky — measured, not taste), 2.2 m/s fall against rain's 30, two-sinusoid tumble whose worst rational closure outlives every flake's time in the box, wind-slant mapped and tested (and invisible in play until a per-zone climate table gives any zone wind above zero — the one wire field it wants, `windDir`, already exists server-side and is discarded), and a two-ramp freezing-point crossfade whose weights sum to one for the whole crossing |
| **"We may need to lower the angles so we can see more what is front"** (owner, 2026-08-13) | **Folded into the zoom slice — one camera-rig opening, owner-tuned.** The 64° pitch was fixed at M3 for the same reason the 36 m distance was, and the same M5b derivation that makes zoom cheap makes pitch cheap: fades and shadow fit follow the constants. The slice ships *both* as live-adjustable (wheel = dolly, a key pair = pitch inside a clamp, ~45–64°) so the owner finds the frame by eye in the world; the numbers they settle on get baked as the new defaults and the derivation test pins them. One consequence to watch: a lower pitch sees farther ahead, so the streaming ring may need a cell of lookahead at the shallow stop — fog already hides the far edge | Rides task #25, immediately behind M5c. ✅ Built the same day (688fd11) |
| **"Ditch the 'can't see what mob is in the next room until we enter it' — we should see the next room as we approach, at a realistic distance"** (owner, 2026-08-13) | **Agreed, and it is a ruling: graphical realism outranks MUD room-scoping here, the pivot's own logic.** The finding is that the seam was already half-crossed: `DESIGN-ranged.md` slice 2 discovered interest management covers rooms-and-neighbours *of rooms and not of bodies* — "a kobold one room west was never sent at all" — and built the `peek` path (`revealed: true`) for exactly these bodies. This ask generalizes peek from look-on-demand to always-on: a third source in `visibleEntities` streaming actors from rooms that share an **open** edge with yours (doors and barriers still hide; the IR's edge classes are the authority), gated on the far room lighting *itself* — outdoor day says yes, a dark cave mouth still says no, so the light model never lies. Same Place only, exactly as peek ruled. **What does not change:** combat, aggro and every command stay room-scoped — seeing a wolf across the field is not the same as being in reach of it, and that distinction is what the coming ranged system monetizes | **Server slice, running now in parallel with M5c** — it touches only `packages/server`, disjoint from the warp's files. The 2D client already draws peeked bodies, so the wire shape is proven. ✅ Built the same day (2715df5); the peek light-gate bug it unearthed — every actor read as a torch — fixed in 9280715 |
| **"Rotate the screen when I hold shift and move the mouse — and always have the camera behind my player"** (owner, 2026-08-13, two messages minutes apart) | **Agreed — the camera learns yaw, one slice.** M3 fixed the yaw deliberately (*"north is up the frame is what makes the 2D map and the 3D view describe the same place"*) and the owner is overturning it with eyes open, which is exactly how that ruling asked to be overturned. Two modes that compose: **Shift+drag orbits** (the gesture the angle lock freed the same hour), and a **follow mode** that keeps the camera at the player's back, easing toward the body's yaw — M7a put float yaw on the wire hours before the owner asked for a camera to use it. The real work is not the rotation, it is everything that quietly assumed north: the streaming ring's lookahead cell must follow the view direction, M6's near-wall fade must re-learn which wall stands "between the camera and the player" under rotation, and cardinal prose ("the sun rises over the *northern* horizon") stops matching screen-up — so a small compass joins the frame. Click-to-move and hold-to-steer are already camera-agnostic through the raycaster and survive untouched | ✅ **Built the same night (`7b904d7`)**, to the owner's fuller third statement — *"the usual 3D setup … like a full 3d game"*. Shift+drag orbits and tilts, **O** follows, **C** homes, a compass joined the view. Every north-assumption found and paid for: the ring became a symmetric disc (**+22 MB of ledger** — the honest price of free rotation at a 96 m ceiling, and `CAMERA_DISTANCE_MAX` is the lever if a machine struggles), the wall fade became a real dot product, the shadow box turns rather than grows. **`FOLLOW_ON_FRESH` was false because W was still world-north** — closed the next morning: the owner asked for camera-relative movement (*"w should always be forward towards what the player is facing"*) and for the camera behind the player, so the objection was retired by the slice below rather than argued away |
| **"The player needs to turn when the map is being rotated so he is facing the direction the map is pointing"** (owner, 2026-08-13) | **Agreed, and it is the slice that finally gives the simulation a real heading.** Two asks the same morning compose into one design: movement becomes camera-relative (W is forward), and *rotating the view turns the character*, camera behind. The first needs no protocol at all — the server already derives `facing` from the direction you move (`sim.ts:2049`, and a click-to-move path feeds the same `intentX/intentY`), so a correctly-rotated world-space vector teaches it the heading for free. **The second cannot be derived**: a player standing still and orbiting produces no movement, so nothing tells the server anything, and other players must see the turn — it is server state or it is a lie. So `Actor` grows a **float heading** beside the cardinal `facing`, fed by a new client intent, throttled, snapping the cardinal for prose and the 2D client exactly as today. This is the *"genuinely smooth server-side yaw is a later slice"* that M7a's `yawOf` docblock promised, arriving because the camera asked for it. **What must survive:** combat's facing lock (it belongs to the opponent, not the camera), `north` staying north in every command and every zone file, and the 2D client compiling untouched. **The owner restated the goal head-up** — *"if he heads west then west needs to swing around to the north position"* — which is follow mode's own behaviour and needs nothing new; but it names the symptom that makes the float heading urgent rather than merely nice: **follow eases toward the server's `facing`, and `facing` is four cardinals**, so a diagonal walk flips `facingOf`'s dominant axis between north and west and the whole world lurches 90° back and forth under the player. A continuous heading is what turns that judder into a turn | **Task #35, immediately behind #34** — #34 (camera-relative W, in flight) is its foundation and owns the same files |
| **"I wonder why the entire room isn't cobblestones"** + **"can we close the gaps between rooms? the rooms should bump right up against the next room so we don't have those bluish looking voids"** (owner, 2026-08-13, one screenshot of a walled city room) | **Two asks, one answer: the ground is the last grey-box surface left.** Checked before briefing. **(1) The floor is a flat colour.** `ARCHETYPE_COLOURS` gives `city` `0x7a7570` and `road` `0x8a7a5c` and that is the whole of it — M5b textured the *props* and M6 textured the *walls*, and the ground kept M3's painted box, so what reads as cobblestone in the screenshot is not the floor at all: it is a handful of `RockPath_*` **kit meshes** dropped by scatter's `edging` layer. A cobbled room wants a **textured ground material**, and both kits already ship what it needs (`PathRocks_Diffuse` in nature, `Floor_Brick`/`Floor_WoodDark` in the village pack, all already imported and served). **(2) The gap is 2 m wide and is *supposed* to be filled** — `ROOM_TILES` 9, `ROOM_GAP` 2, and `chunkPlan`'s own docblock claims *"each room draws half the gap… two adjacent rooms tile a two-metre gap exactly"*. So the bluish voids are that claim failing somewhere: either the half-gap is drawn only on sides that carry an exit, or not at all where the neighbour is a different sector or unloaded. Find which before changing geometry — the fix is cheap if the rule is right and the coverage is wrong, and a redesign if the rule is wrong | **Task #38, behind #37** — same files (`chunkPlan`, `prototypes`, the ground materials). **The owner then re-specified the gap and it is a better design than "close it"** (2026-08-13): *"the gap should be the width of a wall if there is a wall, so that what you see is a wall and not a gap at all. the boundaries of the outside should be controlled by trees or cliff faces or something to show there is an edge but it looks realistic."* That splits one problem into three — a walled boundary where **the wall is the gap** (if `edgeThickness` is narrower than `ROOM_GAP`, that difference *is* the bluish strip), an open boundary where the ground continues, and **the edge of the built world**, which should read as a place ending rather than a cliff of nothing. The third is new work and half-built already: M5a's `boundary` scatter layer exists to put vegetation in the unreachable void outside the room block, which is exactly where a treeline goes. Owner's call: *"do the treeline and cliffs too, don't split it"* |
| **Three proposals, all approved unseen** (Claude, 2026-08-13, under the owner's new standing invitation *"if you think something would look good, just let me know and I will approve or decline"*) | **Every one is an asset already on disk, which is why they were worth raising.** **(1) The grey boxes** — generic room props still draw as M3's untextured boxes, which is what the owner's kobold appeared to stand inside; the Fantasy Props MegaKit's 94 models (barrels, stalls, carts, workbenches, anvils, chests) are downloaded and CC0. **(2) Bald heads** — the Base Characters pack ships 8 hairstyle and eyebrow meshes on the same 65-joint rig, unimported because M7b correctly judged hair a *chargen identity* field that the 2D client still owns; the meshes do not care. **(3) Empty-handed mobs** — players hold real weapons now, but the simulation has no mob equipment list, so a kobold warrior carries nothing; it is the larger one and pays twice, since what a mob visibly wields is also what it can drop | Approved in order: **#39 props**, then **#40 hair**, then mob equipment. Props and hair both queue behind #38 because they share `modelgen`, `chunkPlan`, `prototypes` and `pool` |
