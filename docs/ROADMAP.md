...
| **`quaff`/`drink` for potions, `read` for notes, `eat` for buff foods** (owner, 2026-08-07) | **Agreed, and they are three different sizes.** **`quaff` is nearly free**: `ITEM_POTION` carries exactly the scroll shape in its values (`value[0]` level, `value[1..3]` spell numbers — `do_quaff` reads them as `do_recite` does), so the harvest is the scroll harvest with a different type gate and the delivery is `deliverSpell`; consumed on drink, `uses` already models charges. **`read` needs the extra-description harvest**: the `.obj` parser already walks `E` blocks and `toTemplate` drops the text — carrying it is a catalogue-size decision (16k items × prose) worth measuring first; boards are their own machine (`boards.c`) and wait. **`eat` splits**: the *command* with buff foods (foods casting on consume — the same data path again) is cheap; hunger/thirst stay explicitly-not-scheduled, so food is a buff vector, not a survival clock | **`quaff` next command batch** — it completes the classless-casting triangle scrolls opened. `read` after the extra-description measurement; `eat` with or after quaff, buff-vector only |
